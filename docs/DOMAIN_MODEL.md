...
| 정본 템플릿(Template) | 레이아웃이 검증된 원본 파일(hwpx 30종). 본문만 교체해 재사용 |
...
INV-EXP-003
정본 템플릿은 (단수 × 글자크기 × 문항간격) 조합당 1개가 존재해야 한다(HWPX 30종).
템플릿이 없는 조합은 생성할 수 없다.
```

## 8. 외부 시스템 객체

- **정본 템플릿 자산** — HWPX 30종(`templates.js`), DOCX 30종(`docx-templates.js`). base64 내장.
